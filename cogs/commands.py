...
class Commands(commands.Cog):
    ...
    )  # Permissions required for executing the command
    async def warn(self, ctx, *, member: discord.Member):
        ...
        if member.id != 785550919327940668:
            ...
            try:
                ...
                if str(member.id) in infractions:
                    ...
                    total_warnings = int(
                        df.loc[df["User_ID"] == member.id, "Total_Infractions"]
                    )
                    if warnings > 0:
                        df.loc[df["User_ID"] == member.id, "Infractions"] += 1
                        df.to_csv("database.csv", index=False)
                        warnings = int(
                            df.loc[df["User_ID"] == member.id, "Infractions"]
                        )
                        em = discord.Embed(
                            title="Additional Warning",
                            description=f"You have received an additional warning.. {warnings} total",
                        )
                    else:
                        df.loc[df["User_ID"] == member.id, "Infractions"] = 1
                        df.to_csv("database.csv", index=False)
                        em = discord.Embed(
                            ...
                            description="Your infraction has been recorded..",
                        )
            ...
            em = discord.Embed(
                title="Cannot punish Warden",
                description="Silly, you cannot punish The Warden...",
            )
            await ctx.send(embed=em)
            return
        ...
        try:
            ...
            if int(warnings) >= 3:
                ...
                em = discord.Embed(
                    title="Member Banned",
                    description=f"{member} has been banned for **3 day(s)**",
                )
                await ctx.send(embed=em)
        ...
        try:
            ...
            if str(member.id) in infractions:
                ...
                if warnings > 0:
                    ...
                    df.to_csv("database.csv", index=False)
                    em = discord.Embed(
                        title="Forgiven", description=f"{member} has been forgiven.."
                    )
                else:
                    em = discord.Embed(
                        title="No Interactions",
                        description=f"{member} does not have any infractions to forgive.",
                    )
                await ctx.send(embed=em)
            else:
                em = discord.Embed(
    ...
    async def unban(self, ctx, *, member):
        ...
        try:
            for ban_entry in banned_users:
                ...
                if (user.name, user.discriminator) == (
                    ...
                    em = discord.Embed(
                        ...
                        description=f"Can't find {member} to unban...",
                    )
                    await ctx.send(embed=em)
            ...
            em = discord.Embed(
                title="No Member Found",
                description=f"{member} is not on the banned list.",
            )
            await ctx.send(embed=em)
    ...
    async def whois(self, ctx, *, member: discord.Member):
        ...
        embed = discord.Embed(
            title=member.name, description=member.mention, color=color
        )
    ...
    @commands.command(
        aliases=["purge"]
    )  # This is a very useful command, which can be used, when someone starts spamming a lot in the chat/channel.
    @commands.guild_only()
    ...
    async def slowmode(self, ctx, seconds: int):
        ...
        em = discord.Embed(
            title="Slowmode Set -",
            description=f"Set the slowmode to {seconds} seconds in {ctx.channel.mention}",
    ...
    async def removedelay(self, ctx):
        if ctx.channel.slowmode_delay != 0:
            ...
            em = discord.Embed(
                ...
                description=f"Removed the slowmode of {ctx.channel.mention}!",
                colour=discord.Color.random(),
            )
        else:
            em = discord.Embed(
                title="No Slowmode detected",
                description=f"No slowmode detected in the channel {ctx.channel.mention}",
                colour=discord.Color.random(),
            )
        await ctx.channel.send(embed=em)
